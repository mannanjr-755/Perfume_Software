...
export default function OrdersCrudPage({
  ...
  const previousReloadKey =
    useRef(reloadKey);

  useEffect(() => {
    if (
      previousReloadKey.current ===
      reloadKey
    ) {
      return;
    }
  ...
  /*
   * Load products when order modal opens.
   */
  useEffect(() => {
    if (
      !modalOpen ||
      ...
      if (hasOrderItemsField) {
        ...
        if (!orderItems.length) {
          await Swal.fire({
            ...
            title: 'Add a product',
            text:
              'Please scan or select at least one product before creating this order.',
          });
  ...
  const renderField = (
    ...
    if (
       ...
       * =====================================================
       *
       * Barcode scanner calls this function.
       *
       * We calculate everything INSIDE the functional
       * setForm() updater. This is important because React
       * state updates can be asynchronous.
       *
       * This also makes consecutive barcode scans safer.
       */
      const addProductFromScan = (
        ...
        setForm(
          (current) => {
            ...
            /*
             * Find/add/update the scanned product
             * using the CURRENT cart state.
             */
            const result =
              upsertOrderItem(
                currentItems,
            ...
            /*
             * Show success AFTER the result is calculated.
             */
            if (result.added) {
              toastSuccess(
                'Product added',
            ...
            /*
             * Update cart and total together.
             */
            return {
              ...current,
              [field.name]:
              ...
              {orderItems.map(
                (
                  ...
                  <div
                    ...
                    className="flex gap-3 rounded-lg border divider-border p-2"
                  >
                    <ProductImage
                      src={
                        item.image
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {item.description ? (
                        <p className="mt-0.5 line-clamp-2 text-xs panel-muted">
                          {
                            item.description
                          }
                        </p>
      ...
      <div className="card-surface overflow-hidden">
        <div className="flex flex-col gap-3 border-b divider-border px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold panel-title">
                {title}
              </h2>
